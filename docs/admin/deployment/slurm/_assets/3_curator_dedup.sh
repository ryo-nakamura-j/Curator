...
S3_PARQUET_DIR=s3://BUCKET/CLIPS_PREFIX/iv2_embd_parquet
S3_SEMDEDUPE_DIR=s3://BUCKET/CLIPS_PREFIX/semantic
#
...
srun \
  ...
  --container-mounts "${CONTAINER_MOUNTS}" \
    --  micromamba run -n text_curator \
        python -m nemo_curator.video.pipelines.video.run_pipeline dedup \
        --input-embeddings-path ${S3_PARQUET_DIR} \
        --output-path ${S3_SEMDEDUPE_DIR} \
        --n-clusters 4